...
def base_init_optimizer_state(workload: spec.Workload,
  ...
  lr_schedule_fn = jax_cosine_warmup(workload.step_hint * hyperparameters['training_horizon'], hyperparameters)
...
def base_update_params(workload: spec.Workload,
...
def update_params(workload: spec.Workload,
  ...
  # The step relative to the beginning of this segment of training.
  ...
  # TODO: need to do something to advance the RNG?
  (new_optimizer_state, opt_update_fn), new_params, new_model_state = base_update_params(
    workload=workload,
    current_param_container=current_param_container,
    current_params_types=current_params_types,
    model_state=model_state,
    hyperparameters=HPARAMS[segment],
    batch=batch,
    loss_type=loss_type,
